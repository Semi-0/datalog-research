...
(defn canonical-callable
  ...
  (let [metadata (meta candidate)
        activate
        (:propagators.compiler-2.compiler.basis/application-activate metadata)
        output-selector
        (:propagators.compiler-2.compiler.basis/output-selector metadata)
        contextual?
        (true?
         (:propagators.compiler-2.compiler.basis/contextual-operator metadata))]
    (cond
